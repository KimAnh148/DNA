...
def main():
    original_file = open(destination_path_original_bin, "r")
    encrypted_file = open(destination_path_encrypted, "r")
    decrypted_file = open(destination_path_decrypted_bin, "w")
    key_file = open(destination_path_key, "r")
    ...
    binary_data_decrypted = dna_to_bits(decrypted_text, utils.dna_base_to_two_bits_table)
    
    # convert binary to text
    # text_data = ''.join(chr(int(binary_data_decrypted[i:i+8], 2)) for i in range(0, len(binary_data_decrypted), 8))
    # bit -> bytes an toàn
    byte_array = bytearray(
    int(binary_data_decrypted[i:i+8], 2)
    for i in range(0, len(binary_data_decrypted), 8)
    )

# Lưu đúng dạng byte (không gây ký tự lạ)
    with open("decrypted_text.bin", "wb") as f:
        f.write(byte_array)

    #print("Decrypted text:", text_data)
    # save
    ...
    if  original_text != binary_data_decrypted:
        print("\nDecryption failed.")
    else:
