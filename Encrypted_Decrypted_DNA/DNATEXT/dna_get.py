...
def dna_get(binary_string, key):
    ...
    b_data1 = group_bits(binary_string)
...
def calculate_mse_psnr(bits1, bits2):
    min_len = min(len(bits1), len(bits2))
    bits1 = bits1[:min_len]
    bits2 = bits2[:min_len]

    mse = sum((int(a) - int(b))**2 for a, b in zip(bits1, bits2)) / min_len
...
def main():
    ...
    text_path=r"D:\Encrypted_Decrypted_DNA\DNATEXT\text_8KB.txt"
    ...
    # convert texttext to binary
    binary_data=''.join(format(ord(char), '08b') for char in text_data)
    with open("original_bits.bin", "w") as f:
        f.write(binary_data)
    ...
    encrypted_text = dna_get(binary_data, key)
    ...
    encrypted_binary = dna_to_bits(encrypted_text, utils.dna_base_to_two_bits_table)
    with open("encrypted_bits.bin", "w") as f:
        f.write(encrypted_binary)
    #print("\nTotal execution time:", end - start)

    key_file = open(destination_path_key, "w")
    encrypted_file = open(destination_path_encrypted, "w")
    ...
    original_file.close()
    key_file.close()
    # with open("original_bits.bin", "r") as f:
    #     original_bits = f.read()

    # with open("encrypted_bits.bin", "r") as f:
    #     encrypted_bits = f.read()
    # mse, psnr = calculate_mse_psnr(original_bits, encrypted_bits)

    # print("\nMSE (original vs encrypted):", mse)
    # print("PSNR (original vs encrypted):", psnr)
