...
def main():
    ...
    image_path="D:\Encrypted_Decrypted_DNA\original_image\mandrill.png"
    original_file = open(destination_path_original_bin, "w")
    
    # convert image to binarybinary
    ...
    # convert DNA sequence to image
    # convert DNA sequence to binary
    binary_data_reconstructed = dna_to_bits(encrypted_text, utils.dna_base_to_two_bits_table)

    # convert binary to image
    image = binary_to_image(binary_data_reconstructed)

    # save and show decrypted image
    image.save(os.path.join("encrypted_image", "en_image_mandrill.png"))
    image.show()  
